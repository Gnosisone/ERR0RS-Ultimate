...
class ToolExecutor:
    ...
    async def _execute(self, tool_name: str, cmd_str: str, timeout: int) -> ToolResult:
        ...
        # Award XP for completed tool run
        if status == ToolStatus.SUCCESS:
            try:
                from src.core.progression import award_xp
                event_key = f"run_{tool_name.lower().replace('-','_').replace(' ','_')}"
                award_xp(event_key, target)
                if findings:
                    award_xp("found_vuln", f"{tool_name}: {len(findings)} findings")
            except Exception:
                pass
