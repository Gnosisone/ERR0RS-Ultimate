...
class UniversalToolAdapter:
    ...
    async def _learn_tool(self, tool: DiscoveredTool):
        """Deep learning of tool capabilities — planned AI enhancement.

        Placeholder for future Ollama-driven capability extraction where
        ERR0RS reads --help output, usage examples, and manpage content to
        build a richer tool profile (argument semantics, common workflows,
        dangerous flags). Currently a no-op; tools are adequately profiled
        by the synchronous extraction logic earlier in the pipeline.
        """
        return None
