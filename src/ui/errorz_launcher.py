...
import os, sys, json, time, asyncio, threading, subprocess, webbrowser, re, logging
from http.server import HTTPServer, SimpleHTTPRequestHandler
from pathlib import Path

# ── Logging — /tmp/err0rs_live.log + console ──────────────────────────────────
logging.basicConfig(
    level=logging.INFO,
    format="[ERR0RS] %(levelname)s %(message)s",
    handlers=[
        logging.FileHandler("/tmp/err0rs_live.log", mode="a"),
        logging.StreamHandler(sys.stdout),
    ],
)
log = logging.getLogger("err0rs")
...
# ── Conversation Engine ──────────────────────────────────────────────────────
try:
    from src.core.conversation_engine import get_engine as _get_conv_engine, is_conversational
    CONV_ENGINE = _get_conv_engine()
    print("[ERR0RS] Conversation Engine ready — streaming LLM chat active")
except Exception as _ce:
    CONV_ENGINE = None
    def is_conversational(t): return False
    print(f"[ERR0RS] Conversation Engine unavailable: {_ce}")

# ── Autonomous Agent ──────────────────────────────────────────────────────────
try:
    from src.core.agent import PentestAgent, GOALS
    AGENT_AVAILABLE = True
    print("[ERR0RS] 🤖 Autonomous Agent ready — ReAct loop active")
except Exception as _ae:
    AGENT_AVAILABLE = False
    print(f"[ERR0RS] Agent unavailable: {_ae}")
_active_agent = None
...
try:
    ...
    print("[ERR0RS] 🔍 Autonomous Recon Engine ready — passive + active + web + vuln")
except Exception as _re:
    RECON_ENGINE_AVAILABLE = False
    print(f"[ERR0RS] Recon Engine unavailable: {_re}")
_active_recon = None

# ── AI Threat Detection ───────────────────────────────────────────────────────
try:
    from src.core.threat_detection import ThreatDetector
    THREAT_DETECTION_AVAILABLE = True
    print("[ERR0RS] 🛡️  AI Threat Detection ready — 8 threat signatures, MITRE mapped")
...
# ── Zero-Day Training ─────────────────────────────────────────────────────────
try:
    from src.core.zeroday_training import handle_zeroday_command, ZeroDayTrainer
    ZERODAY_AVAILABLE = True
    print("[ERR0RS] 💀 Zero-Day Training Module ready — 6 research methodology lessons")
except Exception as _zd:
    ZERODAY_AVAILABLE = False
    print(f"[ERR0RS] Zero-Day Training unavailable: {_zd}")
...
def ws_broadcast(msg_type: str, data: str, extra: dict = None):
    ...
    dead = []
    for sid, client in list(_ws_clients.items()):
        try:
            # Schedule the coroutine on the client's event loop
            future = asyncio.run_coroutine_threadsafe(client.send(raw), _WS_LOOP)
...
# WS event loop reference — set when WS server starts
_WS_LOOP = None
...
def route_command(cmd: str) -> dict:
    ...
    if any(t in lower for t in _teach_triggers):
        # Extract topic (everything after "teach me", "explain", "what is", etc.)
        import re as _re
        topic_match = _re.search(
            r'(?:teach me|explain|what is|what are|how (?:do|does|to)|tell me about|lesson on)\s+(.+)',
            lower
        )
        topic = topic_match.group(1).strip() if topic_match else lower
        # Try core teach engine first (more topics)
        try:
            from src.core.teach_engine import format_lesson as _cf, lookup as _cl
            lesson_result = _cf(topic)
            if "No lesson" not in lesson_result:
                return {"stdout": lesson_result, "status": "teach", "source": "teach_engine"}
        except Exception:
            pass
        if TEACH_ENGINE:
    ...
    # ── Juice Shop CTF — forward to operator ─────────────────────────────
    import re as _re2
    if _re2.search(r"\bjuice[- ]?shop\b|\bsolve\s+juice", lower):
        try:
            from src.core.operator import get_operator
            result = get_operator().receive(cmd)
            return result
        except Exception as _je:
            return {"status": "error", "stdout": f"Juice Shop error: {_je}"}
...
async def ws_terminal_handler(websocket):
    ...
    try:
        ...
        # Auto-start demo if --demo flag was passed
        if DEMO_MODE and DEMO_AVAILABLE:
            import asyncio as _asyncio_demo
            _demo_loop = _asyncio_demo.get_event_loop()
            await websocket.send(json.dumps({"type": "system",
                "data": "[ERR0RS] 🎬 DEMO MODE — Starting demonstration in 2 seconds..."}))
            def _demo_broadcast(evt):
                try:
                    if not websocket.closed:
                        _asyncio_demo.run_coroutine_threadsafe(
                            websocket.send(json.dumps(evt)), _demo_loop
                        )
                except Exception:
                    pass
            _start_demo(broadcast_fn=_demo_broadcast, speed=1.2)
        ...
        async for message in websocket:
            ...
            if mtype == "run":
                ...
                if intent.get("raw_cmd"):
                    ...
                    # ── Recon Engine ─────────────────────────────────────
                    _recon_re = __import__('re').compile(
                        r'^(?:recon|scan)\s+(?P<target>\S+)(?:\s+(?P<depth>passive|active|web|full))?',
                        __import__('re').IGNORECASE
                    )
                    _recon_stop = __import__('re').compile(r'^(?:stop recon|recon stop)', __import__('re').IGNORECASE)

                    if _recon_stop.match(clean_cmd):
                        if _active_recon:
                            _active_recon.stop()
                            _active_recon = None
                            await websocket.send(json.dumps({"type":"system","data":"[RECON] Stopped."}))
                        else:
                            await websocket.send(json.dumps({"type":"system","data":"[RECON] No recon running."}))
                        continue

                    _recon_match = _recon_re.match(clean_cmd) if RECON_ENGINE_AVAILABLE else None
                    if _recon_match and not clean_cmd.lower().startswith(('scan ', 'nmap')):
                        # 'recon' command → recon engine; 'scan' alone falls through to tool executor
                        import asyncio as _asyncio_recon
                        _recon_loop = _asyncio_recon.get_event_loop()
                        def _recon_broadcast(evt):
                            try:
                        ...
                        _active_recon.start(
                            ...
                            depth=_recon_match.group("depth") or "full",
                        )
                        continue

                    # ── Threat Detection ──────────────────────────────────────
                    elif clean_cmd.lower() in ("threat monitor", "monitor threats", "start threat detection", "threats"):
                        if THREAT_DETECTION_AVAILABLE:
                            import asyncio as _asyncio_td
                            _td_loop = _asyncio_td.get_event_loop()
                            def _td_broadcast(evt):
                                try:
                            ...
                            await websocket.send(json.dumps({"type":"system","data":"[THREAT] Detection unavailable"}))
                        continue

                    elif clean_cmd.lower() in ("stop threats", "stop threat monitor", "threat stop"):
                        if _threat_detector:
                            _threat_detector.stop()
                            _threat_detector = None
                        continue

                    elif clean_cmd.lower() == "threat scan":
                        if THREAT_DETECTION_AVAILABLE and _threat_detector:
                            alerts = _threat_detector.scan_once()
                            await websocket.send(json.dumps({"type":"system",
                                "data":f"[THREAT] Scan complete — {len(alerts)} alerts"}))
                        else:
                            await websocket.send(json.dumps({"type":"system",
                                "data":"[THREAT] Start monitoring first: 'threat monitor'"}))
                        continue
                    ...
                    # ── Manual / Help ────────────────────────────────────────
                        ...
                        if os.path.exists(manual_path):
                            try:
                                ...
                                pass
                            await websocket.send(json.dumps({"type": "system",
                                "data": f"[ERR0RS] 📖 Manual opened → file://{manual_path}\n"
                                        f"[ERR0RS] Or browse directly: http://127.0.0.1:{HTTP_PORT}/docs/USER_MANUAL.html"}))
                        else:
                            await websocket.send(json.dumps({"type": "system",
                                "data": "[ERR0RS] Manual not found. Try: ls docs/"}))
                        continue

                    # ── Zero-Day Training ─────────────────────────────────────
                    elif clean_cmd.lower().startswith("zeroday") and ZERODAY_AVAILABLE:
                        import asyncio as _asyncio_zd
                        _zd_loop = _asyncio_zd.get_event_loop()
                        def _zd_broadcast(evt):
                            try:
                    ...
                    # ── Agent commands → autonomous agent ────────────────
                    agent_match = None
                    import re as _re_agent
                    _agent_re = _re_agent.compile(
                        r'^(?:agent|autopilot|autonomous|auto|run agent)\s+'
                        r'(?P<target>\S+)'
                        r'(?:\s+(?P<goal>full_chain|web|network|ad|stealth|quick))?',
                        _re_agent.IGNORECASE
                    )
                    _stop_re = _re_agent.compile(r'^(?:stop agent|agent stop|stop autopilot)', _re_agent.IGNORECASE)

                    if _stop_re.match(clean_cmd):
                        if _active_agent:
                            _active_agent.stop("Operator requested stop")
                            _active_agent = None
                            await websocket.send(json.dumps({"type":"system","data":"[AGENT] Stopped."}))
                        else:
                            await websocket.send(json.dumps({"type":"system","data":"[AGENT] No agent running."}))
                        continue

                    elif clean_cmd.lower() in ("agent status", "agent info"):
                        if _active_agent:
                            status = _active_agent.status()
                            await websocket.send(json.dumps({"type":"system","data":json.dumps(status, indent=2)}))
                        else:
                            await websocket.send(json.dumps({"type":"system","data":"[AGENT] No agent running."}))
                        continue

                    else:
                        agent_match = _agent_re.match(clean_cmd)
                    ...
                    if AGENT_AVAILABLE and agent_match:
                        ...
                        # Build broadcast function for this WS session
                        import asyncio as _asyncio_agent
                        _ag_loop = _asyncio_agent.get_event_loop()
                        def _ag_broadcast(evt):
                            try:
                                _asyncio_agent.run_coroutine_threadsafe(
                                    websocket.send(json.dumps(evt)), _ag_loop
                                )
                            except Exception:
                                pass

                        _active_agent = PentestAgent(
                            broadcast_fn=_ag_broadcast,
                            model=model_name,
                        )
                        _active_agent.start(target=ag_target, goal=ag_goal)
                        continue

                    # ── Conversational messages → streaming LLM chat ──────────
                    if CONV_ENGINE and is_conversational(clean_cmd):
                        conv_model = CONV_ENGINE.model if CONV_ENGINE else "llm"
                        await websocket.send(json.dumps({
                            "type": "system",
                            "data": f"[ERR0RS] 🧠 {conv_model} — generating response (first response may take ~10s while model loads)..."
                        }))
                        # Get operator state if available
                        op_state = None
                        try:
                            from src.core.operator import get_operator
                            op_state = get_operator().state
                        ...
                        def _on_token(token):
                            buffer.append(token)
                            # Send every ~60 chars or newline for smooth streaming
                        ...
                        # Run streaming in a thread so we don't block the WS loop
                        import threading as _threading
                        t = _threading.Thread(
                            target=CONV_ENGINE.chat_stream,
                            kwargs={
                    ...
                    # ── Non-conversational → normal route ────────────────
        ...
        try:
            if self.path in ("/health", "/api/health"):
                self._json({"status": "ok", "version": "3.0",
                            "phoenix": PHOENIX_BRIDGE, "narrator": NARRATOR,
                            "ports": {"http": HTTP_PORT, "ws": WS_PORT}})
            elif self.path == "/api/operator/state":
                try:
                    from src.core.operator import get_operator
                    self._json(get_operator().state.summary())
                except Exception as _e:
                    self._json({"status":"error","error":str(_e)})
            elif self.path == "/api/status":   self._json(self._status())
                ...
                try:
                    from src.core.progression import get_progression
                    self._json(get_progression().get_summary())
                except Exception as _pe:
                    self._json({"error": str(_pe), "xp": 0, "level": 0, "level_name": "SCRIPT KIDDIE"})
            elif self.path == "/api/agent/status":
                if _active_agent:
                    self._json(_active_agent.status())
                else:
                    self._json({"running": False, "state": None})
            elif self.path == "/api/agent/goals":
                self._json(GOALS if AGENT_AVAILABLE else {})
            elif self.path == "/api/recon/status":
                if _active_recon:
                    self._json(_active_recon.status())
                else:
                    self._json({"running": False})
            elif self.path == "/api/threat/status":
                if _threat_detector:
                    self._json(_threat_detector.status())
                else:
                    self._json({"running": False, "alerts": 0})
            elif self.path == "/api/zeroday/topics":
                if ZERODAY_AVAILABLE:
                    from src.core.zeroday_training import ZERODAY_CURRICULUM
                    self._json({"topics": list(ZERODAY_CURRICULUM.keys())})
                else:
                    self._json({"topics": []})
            elif self.path == "/api/onboarding":
                try:
                    from src.core.onboarding import is_first_run, get_onboarding_payload
                    if is_first_run():
                        self._json({"first_run": True, "payload": get_onboarding_payload()})
                    else:
    ...
    def do_POST(self):
        try:
            ...
            if self.path == "/api/onboarding/complete":
                try:
                    from src.core.onboarding import mark_onboarding_complete, save_preferences, load_preferences
                    from src.core.progression import award_xp
                    prefs = load_preferences()
                    prefs["agreed_to_tos"] = payload.get("agreed", True)
                    prefs["skill_level"]   = payload.get("skill_level", 0)
                    prefs["name"]          = payload.get("name", "Operator")
                    prefs["mode"]          = "guided" if prefs["skill_level"] <= 1 else "standard"
                    mark_onboarding_complete(prefs)
                    award_xp("daily_login", "first run")
                ...
                try:
                    from src.core.progression import award_xp
                    event   = payload.get("event", "ask_question")
                    context = payload.get("context", "")
                    result  = award_xp(event, context)
                    self._json(result)
                except Exception as _pe:
            ...
            # ── RP2040 / Pico BadUSB Programmer ──────────────────────────────
                ...
                #  POST {"msg": "run an nmap syn scan on 192.168.1.0/24"}
                #  → operator.receive() returns action + findings + suggestions
                try:
                    from src.core.operator import get_operator
                    msg = (payload.get("msg") or payload.get("message") or "").strip()
                    if not msg:
                        self._json({"status":"empty","reply":"Say something, operator."})
                    else:
                        # Detect long-running intents and offload to a thread.
                        # These would block the HTTP server for 60-120+ seconds.
                        import re as _re
                        _LONG_PATTERNS = [
                            r"\bsolve\s+juice[- ]?shop\s+(all|$)",   # "solve juice-shop all" or bare "solve juice-shop"
                            r"\bjuice[- ]?shop\s+all\b",             # "juice-shop all"
                            r"\brun\s+killchain\b",
                            r"\bauto\s+recon\b",
                        ]
                        _ml = msg.lower().strip()
                        _is_long = any(_re.search(p, _ml) for p in _LONG_PATTERNS)

                        op = get_operator()

                        if _is_long:
                            # Fire in background thread, broadcast results via WS
                            import concurrent.futures as _cf
                            import threading as _th
                            def _bg():
                                try:
                                    r = op.receive(msg)
                                    summary = r.get("reply", "Done")
                                    ws_broadcast("operator", summary, {"status": "complete", "data": r})
                                except Exception as _e:
                                    ws_broadcast("error", str(_e), {"status": "error"})
                            _th.Thread(target=_bg, daemon=True).start()
                            self._json({
                                "status": "running",
                                "reply": f"⚙️ Running in background — watch the live terminal for results. ({msg[:60]})",
                            })
                        else:
                            result = op.receive(msg)
                            self._json(result)
                except Exception as _opErr:
                    log.error(f"Operator error: {_opErr}", exc_info=True)
                    self._json({"status":"error","error": str(_opErr)})

            elif self.path == "/api/operator/state":
                # GET/POST the current operator state summary
                try:
                    from src.core.operator import get_operator
                    self._json(get_operator().state.summary())
                except Exception as _opErr:
                    ...
                    try:
                        ...
                        if action in LIVE_ACTIONS and dry_run:
                            ...
                            with concurrent.futures.ThreadPoolExecutor(max_workers=1) as ex:
                                ...
                                result, _lat_log = future.result(timeout=7)
                    ...
                    try:
                        ...
                        if action in LIVE_ACTIONS and dry_run:
                            ...
                            with concurrent.futures.ThreadPoolExecutor(max_workers=1) as ex:
                                ...
                                result, _wir_log = future.result(timeout=7)
                            d = dataclasses.asdict(result)
                            d["_log"] = _wir_log[:500]
                            self._json({"status":"ok","result": d})
                    except concurrent.futures.TimeoutError:
...
def start_server():
    ...
    # ── ERR0RS Operator Brain — wire ws_broadcast into the operator ──────────
    try:
        from src.core.operator import get_operator
        _op = get_operator()
        _op.set_broadcast(ws_broadcast)
        log.info("✅ Operator brain online — natural language + auto-chain ready")
    except Exception as _opE:
        log.warning(f"Operator brain failed to init: {_opE}")

    # Pre-warm the conversation engine model so first query is fast
    if CONV_ENGINE:
        import threading as _boot_t
        def _boot_warm():
            try:
                CONV_ENGINE._warmup()
            except Exception:
                pass
        _boot_t.Thread(target=_boot_warm, daemon=True).start()
    ...
    # ── Open browser (DISPLAY=:0 ensures it appears on Pi desktop) ──────────
    try:
        ...
        for _br in ["firefox", "chromium", "chromium-browser", "epiphany"]:
            if shutil.which(_br):
                _sp.Popen([_br, url], env=_env,
                          stdout=_sp.DEVNULL, stderr=_sp.DEVNULL,
                          start_new_session=True)
                log.info(f"Browser opened: {_br} → {url}")
                _opened = True
                break
        if not _opened:
            log.warning("No browser found — open manually: " + url)
    except Exception as _be:
        log.warning(f"Browser open failed: {_be}")
...
try:
    from src.core.demo_mode import start_demo as _start_demo
    DEMO_AVAILABLE = True
...
def main():
    ...
    PURPLE='\033[0;35m'
    demo_tag = f"{PURPLE}  [DEMO MODE — localhost only]{N}\n" if DEMO_MODE else ""
    print(f"""
{O}  ███████╗██████╗ ██████╗  ██████╗ ██████╗ ███████╗{N}
{O}  ██╔════╝██╔══██╗██╔══██╗██╔═███╗██╔══██╗██╔════╝{N}
{Y}  █████╗  ██████╔╝██████╔╝██║██╔╝██████╔╝███████╗{N}
...
{R}  ███████╗██║  ██║██║  ██║╚██████╔╝██║  ██║███████║{N}
{R}  ╚══════╝╚═╝  ╚═╝╚═╝  ╚═╝ ╚═════╝ ╚═╝  ╚═╝╚══════╝{N}
{C}        AI PENTEST PLATFORM + SECURITY COACH  v3.2{N}
{O}  ═══════════════════════════════════════════════════{N}
{demo_tag}  {C}Root:{N}       {ROOT_DIR}
