...
unified registry conforming to tool_registry.schema.json:
  ...
  1. src/tools/tool_registry.json        — structured legacy registry
  2. src/core/teach_engine.py  LESSONS   — alt teaching prose + concepts

Output:
  src/tools/tool_registry.v2.json        — Unified registry
  src/tools/concepts.v2.json             — Concept entries (CIA, OWASP, MITRE)
...
CATEGORY_MAP = {
    "recon": "recon", "web": "web", "exploitation": "exploitation",
    "exploit": "exploitation", "post-exploit": "post-exploit",
    "post-exploitation": "post-exploit", "persistence": "persistence",
    "c2": "c2", "wireless": "wireless", "credentials": "credentials",
    "cracking": "credentials", "forensics": "forensics", "reverse": "reverse",
    "reverse-engineering": "reverse", "evasion": "evasion",
    "social": "social-engineering", "social-engineering": "social-engineering",
    "phishing": "social-engineering", "mobile": "mobile", "cloud": "cloud",
    "container": "container", "ad": "ad", "active-directory": "ad",
    "network": "network", "hardware": "hardware", "ids": "ids",
    "monitoring": "ids", "utility": "utility", "general": "utility",
}
...
CONCEPT_CATEGORIES = {
    "cia": "doctrine", "cis": "framework", "incident-response": "phase",
    "kill-chain": "model", "mitre": "framework", "owasp": "standard",
    ...
    if canonical in {k.lower() for k in lessons}:
        ...
        for flag_name, flag_teach in lesson.get("flags", {}).items():
            if flag_name in entry["flags"]:
                ...
                entry["flags"][flag_name] = {
                    "label": flag_name, "desc": "", "example": "", "teach": flag_teach,
                    "mechanism": "", "detection": "", "evasion": "", "risk": "moderate",
                }
    return entry
...
def migrate_concept(key: str, lesson: dict) -> dict:
    ...
    for component_key in ("pillars", "phases", "controls", "items",
        ...
        if component_key in lesson:
            ...
            break
    # Fallback: LESSONS concept entries reuse the tool template, using `flags`
    # to hold framework components like OWASP A01-A10
...
def main():
    ...
    # teach_engine.py may or may not exist depending on repo state
    try:
        lessons = load_lessons()
    except (FileNotFoundError, RuntimeError) as e:
        print(f"  ⚠  Could not load LESSONS: {e}")
        print(f"  Continuing with legacy registry only.")
        lessons = {}
    ...
    if empty_intro:
        print(f"  ⚠  tools missing teach_intro: {len(empty_intro)}")
